...
% Record the folders containing DAPI and Phase images
phase_folder = dir(uigetdir('', 'Choose the folder containing the phase files'));
dapi_folder = dir(uigetdir('', 'Choose the folder containing the DAPI max projections'));
results_folder = uigetdir('', 'Choose the folder where you want to save the results');
% Loop over all of phase and dapi images
for image_no = 3:length(phase_folder)
im_phase = imread(strcat(phase_folder(image_no).folder,'\',phase_folder(image_no).name)); 
im_dapi = imread(strcat(dapi_folder(image_no).folder,'\',dapi_folder(image_no).name)); 
file = dapi_folder(image_no).name;

%% clear export_file so that it does not interfere with next iteration
clear export_file;
...
size_image = size(foci_image);
foci_image = foci_image(1:size_image(1),1:size_image(2));
file_prefix = split(file, '.');
file_prefix = file_prefix{1};

imwrite(foci_image, strcat(results_folder,strcat('FociMAX', strcat(file_prefix, '.tif'))), 'Compression', 'none');

writetable(export_file, strcat(results_folder,strcat(file_prefix, '.csv')));

% The last end call for the original for loop
end
